...
var ProjectStore = class ProjectStore {
    ...
    constructor(persistence) {
        ...
        const cached = persistence.readJSON(CACHE_FILE, null);
        const projects = cached && cached.projects ? cached.projects : [];
    ...
    _set(id, field, value) {
        ...
        if (!p) {
            L.error("_set: unknown project " + id);
            return;
        }
        ...
        const queued = this._queuedFor(key);
    ...
    _pump() {
        ...
        const key = entry.id + "\0" + entry.field;
        this._inFlight = true;

        const push =
            entry.field === "inWorkspace"
        ...
        push.then(() => {
            ...
            .catch((e) => {
                ...
                if (this._onWriteError) {
                    try {
                        this._onWriteError(entry.id, entry.field, e);
                    } catch (err) {}
                }
            })
            .then(() => {
    ...
    merge(remoteProjects, protectedIds) {
        ...
        for (let i = 0; i < remoteProjects.length; i++) {
            const r = remoteProjects[i];
            remoteIds.add(r.id);
            const local = this._byId.get(r.id);
            if (!local) {
            ...
            // Deck fields: remote wins unless a local write is pending; but an
            // in-deck id is never downgraded to false (the deck owns membership).
            if (!this._dirty.has(r.id + "\0inWorkspace")) {
                const remoteInW = keep.has(r.id) && !r.inWorkspace ? true : r.inWorkspace;
    ...
    _notify(reason) {
        if (this._onChange) {
            try {
                this._onChange(reason);
            } catch (e) {
                L.error("onChange cb: " + e.toString());
            }
        }
    }
