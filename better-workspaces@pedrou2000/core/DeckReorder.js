...
var DeckReorder = class DeckReorder {
    ...
    reorderProject(from, to) {
        ...
        const curLoc = c.currentLocation();

        const counts = c.state.counts();
        const blocks = []; // blocks[oldProjIdx] = [wsObj per local]
        for (let pi = 0; pi < n; pi++) {
            const base = Mapping.offsetOf(counts, pi);
            const objs = [];
        ...
        const order = c.state.moveProject(from, to); // order[newPos] = oldProjIdx
        if (!order) return false;

        const desired = [];
        for (let newPos = 0; newPos < n; newPos++) {
            const objs = blocks[order[newPos]];
        ...
        if (curLoc) {
            const newActivePos = order.indexOf(curLoc.projectIdx);
            const flat = Mapping.globalIndex(c.state.counts(), newActivePos, curLoc.localIdx);
    ...
    moveActiveProjectBy(delta) {
        const c = this._c;
        const from = c.state.activeProjectIdx;
        const to = from + delta;
