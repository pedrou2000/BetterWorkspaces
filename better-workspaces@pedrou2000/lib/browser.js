...
function _defaultBrowserBin() {
    ...
    _cachedBin = null;
    try {
        const [ok, out] = GLib.spawn_command_line_sync("xdg-settings get default-web-browser");
        if (ok && out) {
            const desktop = (out instanceof Uint8Array ? ByteArray.toString(out) : String(out))
                .trim()
                .toLowerCase();
...
function openUrlNewWindow(url) {
    const bin = _defaultBrowserBin();
    // Fall back to xdg-open (right browser, but a new tab) when unrecognized.
    const cmd = bin ? [bin, "--new-window", url] : ["xdg-open", url];
    try {
        Util.spawn(cmd);
    } catch (e) {
